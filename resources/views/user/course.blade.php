                ...
                <div class="col-md-8 mb-3">
                    <div class="card h-100" style="border-radius:20px;">
                        <div class="card-body d-flex flex-column" style="text-align: center;border-radius:20px; background:linear-gradient(rgba(0, 0, 0, 0.6),rgba(0, 0, 0, 0.6)), url({{ asset('storage/' . $course->image) }});background-position: center;">
                            ...
                            @elseif($enrollment->status == 0)
                                @if (is_array($transaction) || is_object($transaction))
                                    ...
                                    <div class="mt-auto" style="display: flex; justify-content: space-between; gap:5px">
                                        <a data-toggle="tooltip" title="You will be notified to your email when the transaction has been approved" disabled
                                            style="border-radius:20px; width: 100%" class="mt-auto btn btn-light">
                                            Pending, Waiting for the approval process. Please, be patient. Thank you :)
                                        </a>
                                    </div>
                                    @elseif($transaction->approve == 1)
                                    <div class="mt-auto" style="display: flex; justify-content: space-between; gap:5px">
                                        <a data-toggle="tooltip" title="Your transaction has been approved. You can contact Us if this happens we will change the course access" disabled
                                            style="border-radius:20px; width: 100%" class="mt-auto btn btn-secondary">
                                            Wait for the course access change. Thank you :)
                                        </a>
                                    </div>
                                    @elseif($transaction->approve == 2)
                                    <div class="mt-auto" style="display: flex; justify-content: space-between; gap:5px">
                                        <a data-toggle="tooltip" title="This happend because of an issue with your transaction, Contact Us to Fix it." disabled
                                            style="border-radius:20px; width: 100%" class="mt-auto btn btn-danger">
                                ...
                                @else
                                    <div class="mt-auto" style="display: flex; justify-content: space-between; gap:5px">
                                        ...
                                        <span data-toggle="modal" data-target="#paying">
                                            <a data-toggle="tooltip" title="Pay for this Course and Get Full Access" style="border-radius:20px; width: 100%"
                                                class="mt-auto btn btn-success">Buy</a>
                                        </span>
                                        <div class="modal fade" id="paying" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog"
                                            aria-labelledby="paying" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered " role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLongTitle">Buy {{$course->course_name}}</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="alert alert-info" role="alert">
                                                            <h4 class="alert-heading">Buying Course {{$course->course_name}}</h4>
                                                        ...
                                                        <div style="text-align: left" class="m-3">
                                                            <form action="{{route('user.payCourse')}}" id="payForm" method="post"
                                                                enctype="multipart/form-data">
                                                                @csrf
                                                                <label for="ID_bank">Bank</label>
                                                                <select id="bank" class="form-control mb-2" type="text" name="ID_bank">
                                                                    <option id="nothing" value="0" selected disabled hidden>Click here to choose a Bank From
                                                                        the List Below</option>
                                                                    @foreach ($banks as $bank)
                                                                    <option id="ID_bank" value="{{$bank->ID_bank}}">{{$bank->bank_name}} -
                                                                        {{$bank->no_account}}</option>
                                                                    @endforeach
                                                                </select>
                                                                <label for="image">Proof</label>
                                                                <div class="alert alert-warning p-2 mt-2">
                                                                    Send Rp.{{$course->price}} to the bank account you have chosen.
                                                                    <br> Upload the receipt to as a proof. Image only
                                                                    <input type="file" style="padding: 4px" class="form-control mt-3" name="image"
                                                                        accept="image/*">
                                                                </div>
                                                            ...
                                                            <table class="table custom-table">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    <tr scope="row">
                                                                        ...
                                                                        <td>
                                                                            {{$bank->no_account}}
                                                                            <small class="d-block">Official CariTau Bank Account</small>
                                                                        </td>
                                                                    </tr>
                                                                    @endforeach
                                                                    @else
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <a onclick="document.getElementById('payForm').submit();" type="button"
                                                                class="btn btn-success">Yes, buy</a>
                                                        </div>
                                                        @else
                                                        <div class="alert alert-info" role="alert">
                                                            <h4 class="alert-heading">We are Sorry! We have a Problem Right Now</h4>
                                        ...
                                        <span data-toggle="modal" data-target="#unenrolling">
                                            <a data-toggle="tooltip" title="Unenroll in This Course" style="border-radius:20px;width: 100%"
                                                class="mt-auto btn btn-danger">Unenroll</a>
                                        </span>
                                        <div class="modal fade" data-backdrop="static" data-keyboard="false" id="unenrolling" tabindex="-1" role="dialog"
                                            aria-labelledby="unenrolling" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                            ...
                            @endif
                            @else
                            <a data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#enrolling" style="border-radius:20px;" class="mt-auto btn btn-outline-success">Enroll</a>
