                    ...
                    <div class="card-body">
                        <div class="float-right" style="cursor: pointer;">
                            <a style="text-decoration: none;cursor: pointer"
                                onclick="$('#edit').toggle('slow'); $('#info').hide('fast'); return false;">Edit <i
        ...
        <div class="row gutters-sm" id="edit" style="display: none">
            <div class="col-md-4 mb-3">
                            ...
                            <div class="mt-3">
                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                    ...
                                    <form method="post" style="display: none;"
                                        action="{{ route('userLecturer.updateImage', Auth::user()) }}"
                                    ...
                                    <form style="display: none" method="POST"
                                        action="{{ route('userLecturer.restoreImage', Auth::user()) }}" id="restore">
                                        @csrf
                                    </form>
                                </div>
                            </div>
                ...
                <div style="margin-top: 10px;" class="justify-content-center">
                    <center><button style="animation: drop 0.8s" class="btn btn-outline-danger" data-toggle="modal"
                            data-target="#deleteAcount">Delete Account</button></center>
                    <form id="delete" style="display: none" action="{{ route('userLecturer.delete', Auth::user()) }}"
                        method="POST">
                        @csrf
                        @method('DELETE')
                    </form>
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle">Delete your account
                                        {{Auth::user()->name}}</h5>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">No, I'm not so
                                        sure</button>
                                    <button onclick="document.getElementById('delete').submit();" type="button"
                                        class="btn btn-danger">Yes, Sure. Delete My Account</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-md-8">
                <div class="card mb-3" style="border-radius:20px;">
                    <form method="post" action="{{ route('userLecturer.update', Auth::user()) }}"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="card-body">
                            <div class="float-right" style="cursor: pointer;">
                                <a style="text-decoration: none ;cursor: pointer"
                                    onclick="$('#info').toggle('slow'); $('#edit').hide('fast'); return false;">Close <i
